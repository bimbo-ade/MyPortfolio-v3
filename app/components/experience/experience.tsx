import React from "react";

const Experience = () => {
  return (
    <div className="group">
      <div className="  mt-[30px] tablet:flex tablet:my-[50px]   laptop:p-6 transition duration-120 laptop:hover:bg-[#9EC3FA] laptop:hover:bg-opacity-[0.014]">
        <p className="mb-[10px] tracking-[1px] tablet:mr-[20px] text-[12px]">
          {" "}
          Mar 2023 - PRESENT
        </p>
        <div className=" tablet:w-[80%] laptop:w-[70%]  ">
          {" "}
          <p className="text-[#53DD89] text-[16px] desktop:text-[16px]">
            {" "}
            Frontend Engineer . MAX.NG
          </p>
          <p className="hidden text-[14px] opacity-[0.6] tablet:block">
            Frontend Engineer
          </p>
          <p className=" mt-[10px]  text-[16px]  ">
            In my role, I actively collaborated with a diverse team of frontend
            and backend developers, designers, and product managers to
            successfully bring our product to life. I played a pivotal role in a
            solo project, taking charge of everything from UI implementation to
            API integration, ensuring a seamless and comprehensive delivery.
            Additionally, I led a collaborative effort to overhaul and modernize
            our existing app, enhancing its user-friendliness and overall user
            experience. These experiences showcase my ability to work both
            independently and as part of a collaborative team to achieve
            successful project outcomes.
          </p>
          <div className="flex mt-[20px] flex-wrap gap-2 desktop:gap-4">
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.3] rounded-[40px]">
              {" "}
              Nextjs
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Reactjs{" "}
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Typescript{" "}
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Chakra Ui{" "}
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Flutter{" "}
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              HTML CSS & SCSS{" "}
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Tailwind CSS{" "}
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Styled Components
            </p>{" "}
          </div>
        </div>
      </div>{" "}
      <div className="  my-[80px] tablet:flex tablet:my-[80px]   laptop:p-6 transition duration-120 laptop:hover:bg-[#9EC3FA] laptop:hover:bg-opacity-[0.014]">
      <p className="mb-[10px] tracking-[1px] tablet:mr-[20px] text-[12px]">
          {" "}
          Oct 2022 - Jan 2023
        </p>
        <div className=" tablet:w-[80%] laptop:w-[70%]  ">
          {" "}
          <p className="text-[#53DD89] text-[16px] laptop:text-[16px]">
            Frontend Engineer . HNG
          </p>
          <p className="hidden text-[14px] opacity-[0.6] tablet:block">
            Frontend Engineer
          </p>{" "}
          <p className=" mt-[10px]  text-[16px]  ">
            As a pivotal member in the creation of our flagship product at HNG,
            I collaborated closely with frontend and backend developers,
            designers, and product managers. Together, we navigated the dynamic
            landscape, seamlessly integrating design elements, and translating
            concepts into a cohesive product. This collaborative synergy not
            only ensured a successful product launch but also fostered an
            innovative and respectful team culture.
          </p>
          <div className="flex mt-[20px] flex-wrap gap-2 desktop:gap-4">
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Reactjs{" "}
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              HTML & CSS{" "}
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.3] rounded-[40px]">
              {" "}
              JavaScript
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Tailwind CSS{" "}
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.3] rounded-[40px]">
              {" "}
              Storybook
            </p>{" "}
          </div>
        </div>
      </div>{" "}
      <div className="  my-[80px] tablet:flex tablet:my-[80px]   laptop:p-6 transition duration-120 laptop:hover:bg-[#9EC3FA] laptop:hover:bg-opacity-[0.014]">
      <p className="mb-[10px] tracking-[1px] tablet:mr-[20px] text-[12px]">
          {" "}
          Sept 2021 - Oct 2022
        </p>
        <div className=" tablet:w-[80%] laptop:w-[70%]  ">
          {" "}
          <p className="text-[#53DD89] text-[16px] laptop:text-[16px]">
            {" "}
            UI Developer . Citiserve Limited
          </p>
          <p className="hidden text-[14px] opacity-[0.6] tablet:block">
            UI Developer & Frontend Engineer
          </p>
          <p className=" mt-[10px]  text-[16px]  ">
            As a key member of the development team at [Company Name], I played
            a vital role in translating design concepts into functional web
            applications. I spearheaded design recreation, ensuring seamless
            implementation with meticulous attention to detail. Collaborating
            closely with the marketing team, I translated strategies into
            impactful flyer designs, contributing to successful campaigns and
            brand awareness. Engaging with customers directly, I gathered
            valuable feedback, conducted outreach calls, and played a key role
            in tracking product growth. These experiences sharpened my technical
            skills and provided a holistic understanding of the intersection
            between design, marketing, and customer engagement.
          </p>
          <div className="flex mt-[20px] flex-wrap gap-2 desktop:gap-4">
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.3] rounded-[40px]">
              {" "}
              ReactJS
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              HTML & CSS
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              JavaScript
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Styled Components
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Figma
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              UI Design
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Adobe
            </p>{" "}
            <p className="px-[14px] py-[4px] bg-[#53DD89] text-[#53DD89] text-[12px] bg-opacity-[0.2] rounded-[40px]">
              {" "}
              Canva
            </p>{" "}
          </div>
        </div>
      </div>
    </div>
  );
};

export default Experience;
